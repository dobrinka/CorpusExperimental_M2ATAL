...
myfile = codecs.open("test", "w", "utf-8")

if not opt_noheader:
	print "file\tF\tP\tR\tF.tag\tP.tag\tR.tag\tTP\tFP\tFN\tTP.tag\tFP.tag\tFN.tag"
	myfile.write ("file\tF\tP\tR\tF.tag\tP.tag\tR.tag\tTP\tFP\tFN\tTP.tag\tFP.tag\tFN.tag\n")
n_processed = 0
...
for filename in files:
	...
	if not opt_total:
		...
		myfile.write(filename + "\t" + ("%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%d\t%d\t%d\t%d\t%d\t%d\n" % eval_list))
